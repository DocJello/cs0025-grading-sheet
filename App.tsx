                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                             <NavLink
                                ...
                                onClick={() => setCurrentPage('group-management')}
                            />
                            <NavLink
                                icon={<ListIcon className="w-6 h-6" />}
                                label="Masterlist"
                                isActive={currentPage === 'masterlist'}
                                onClick={() => setCurrentPage('masterlist')}
                            />
                             <NavLink
...
const AppContent: React.FC = () => {
    const { currentUser, isLoading, currentPage, navigateToGradeSheet } = useAppContext();
    ...
    const renderPage = () => {
        switch (currentPage) {
            ...
            case 'setup':
                 return <Setup />; // Allow access even if logged in, for re-initialization
            default:
                return <Dashboard navigateToGradeSheet={navigateToGradeSheet} />;
        }
    };
